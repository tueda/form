#-
* Print a rough estimate for (pseudo) form units per hour on the current
* machine. The definition of 1 form unit is:
*
*   1fu = performing a trace with 14 Dirac's gamma matrices 3600 times.
*
* The modifier "pseudo" comes from the fact that the form units are originally
* measured in the real time for parallel versions but we here use the CPU time.
*
...
Format 120;
  ...
  #write "  Roughly `fuph' (pseudo) form units per hour per core"
  ...
  #write "  Roughly `fuph' form units per hour"
#endif
