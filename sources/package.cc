...
/**
 * Ensures that the specified package is available. The package is specified as
 * [<site>/]<group>/<name>@<tag>.
 */
extern "C" int EnsurePackage(UBYTE* s) {
  std::string repo;
  std::string group;
  std::string name;
  std::string version;

  int err = SplitPackageNameComponents(std::string((const char*)s), &repo,
                                       &group, &name, &version);
  if (err) return err;

  return EnsurePackage(repo, group, name, version);
}
